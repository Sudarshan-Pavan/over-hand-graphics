...
def add_jarvis_overlay(img, hand_landmarks, finger_states, line_thickness=2):
    ...
    draw_static_circle(img, palm_center_px, radius=40, color=(255, 255, 200), thickness=2)
    draw_static_circle(img, palm_center_px, radius=20, color=(255, 255, 0), thickness=2)
    ...
    for i, state in enumerate(finger_states):
        if state == 1:  # Finger open
            ...
            # Determine start point (outer JARVIS circle edge)
            base_x, base_y = joint_coords[0]
            # Find direction vector from palm center to base joint
            dir_x, dir_y = base_x - palm_center_px[0], base_y - palm_center_px[1]
            mag = math.sqrt(dir_x**2 + dir_y**2)
            if mag != 0:
                dir_x, dir_y = dir_x / mag, dir_y / mag
            start_x = int(palm_center_px[0] + dir_x * 40)  # 40 = outer circle radius
            start_y = int(palm_center_px[1] + dir_y * 40)
            start_point = (start_x, start_y)

            # Combine start + finger joints
            all_points = [start_point] + joint_coords
