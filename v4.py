...
# partical circles for jarvis !!
def draw_custom_arc_circle(img, center, radius, color, thickness, gaps=[]):
    """
    Draws a circle with multiple gaps.
    gaps: list of (start_angle, end_angle) in degrees, clockwise from horizontal right.
    Example: [(30, 60), (150, 180)]
    """
    ...
    for gap_start, gap_end in gaps:
        ...
        if gap_start > current_angle:
            cv2.ellipse(img, center, (radius, radius), 0, current_angle, gap_start, color, thickness, cv2.LINE_AA)
        current_angle = gap_end  # skip the gap

    # Draw the last segment (from last gap to 360)
    if current_angle < 360:
        cv2.ellipse(img, center, (radius, radius), 0, current_angle, 360, color, thickness, cv2.LINE_AA)
...
def add_jarvis_overlay(img, hand_landmarks, finger_states, line_thickness=2):
    ...
    gaps = [(30, 60), (150, 180), (300, 330)]
    draw_custom_arc_circle(img, palm_center_px, radius=40, color=(255, 255, 200), thickness=2, gaps=gaps)
